...
$nbrpass = $_SESSION['filtres']['nbrpass'];

$prix_max = $conn->real_escape_string($_POST['rangeslider']);
$compagnies = isset($_POST['compagnies']) ? $_POST['compagnies'] : [];
$heures = isset($_POST['heure']) ? $_POST['heure'] : [];
// query: affiche la requette pour utilisateur query_base non
$query_base = "";
if ($filtres['al'] == 'aller-retour') {
    $query_base = "SELECT departure_time, arrival_time, flight_duration, price, departure_airport, arrival_airport, compagnie, ville_depart, ville_arrive, date_depart, flight_type, img_compagnie FROM reto WHERE price <= $prix_max AND ville_depart = '{$conn->real_escape_string($filtres['departure'])}' AND ville_arrive = '{$conn->real_escape_string($filtres['arrival'])}' AND date_depart = '{$conn->real_escape_string($filtres['depdate'])}' AND date_arrive = '{$conn->real_escape_string($filtres['arrivdate'])}'";
} elseif ($filtres['al'] == 'Direct') {
    $query_base = "SELECT departure_time, arrival_time, flight_duration, price, departure_airport, arrival_airport, compagnie, ville_depart, ville_arrive, date_depart, flight_type, img_compagnie FROM flights WHERE price <= $prix_max AND ville_depart = '{$conn->real_escape_string($filtres['departure'])}' AND ville_arrive = '{$conn->real_escape_string($filtres['arrival'])}' AND date_depart = '{$conn->real_escape_string($filtres['date_depart'])}'";
} elseif ($filtres['al'] == 'Une escale') {
    $query_base = "SELECT departure_time, arrival_time, flight_duration, price, departure_airport, arrival_airport, compagnie, ville_depart, ville_arrive, date_depart, flight_type, img_compagnie FROM mul WHERE price <= $prix_max AND ville_depart = '{$conn->real_escape_string($filtres['departure1'])}' AND ville_arrive1 = '{$conn->real_escape_string($filtres['arrival1'])}' AND date_depart = '{$conn->real_escape_string($filtres['depdate1'])}' AND date_arrive1 = '{$conn->real_escape_string($filtres['arrivdate1'])}' AND ville_depart2 = '{$conn->real_escape_string($filtres['departure2'])}' AND ville_arrive = '{$conn->real_escape_string($filtres['arrival2'])}' AND date_depart2 = '{$conn->real_escape_string($filtres['depdate2'])}' AND date_arrive2 = '{$conn->real_escape_string($filtres['arrivdate2'])}'";
} else {
    // Default query to avoid errors
    $query_base = "SELECT departure_time, arrival_time, flight_duration, price, departure_airport, arrival_airport, compagnie, ville_depart, ville_arrive, date_depart, flight_type, img_compagnie FROM flights WHERE 1=1";
}
...
if (!empty($compagnies)) {
    $compagnies_list = implode("','", array_map([$conn, 'real_escape_string'], $compagnies));
    $query .= " AND compagnie IN ('$compagnies_list')";
}
...
if (!empty($heures)) {
    ...
    foreach ($heures as $heure) {
        switch ($heure) {
            case 'early':
                $heures_conditions[] = "(departure_time BETWEEN '00:00:00' AND '06:00:00')";
                break;
            case 'morning':
                $heures_conditions[] = "(departure_time BETWEEN '06:00:00' AND '12:00:00')";
                break;
            case 'afternoon':
                $heures_conditions[] = "(departure_time BETWEEN '12:00:00' AND '18:00:00')";
                break;
            case 'night':
                $heures_conditions[] = "(departure_time BETWEEN '18:00:00' AND '23:59:59')";
                break;
        }
    }
    $query .= " AND (" . implode(" OR ", $heures_conditions) . ")";
}
...
<body>
    ...
    <main>
        <section class="w-full flex justify-center relative py-20">
            <div class="w-full container center">
                <div class="">
                    <div data-v-5437da4f="" class="container flex justify-between px-20 rounded bg-white h-[80px]" style="margin-top: 80px;height: 100px;">
                            ...
                            <div data-v-5437da4f="" class="gap-x-20 flex items-center absolute top-1/2 transform -translate-y-1/2 py-[20px]">
                                ...
                                ?>
                                    <div data-v-5437da4f="" class="flex flex-col gap-y-5" data-testid="flightResult-search-detail">
                                        ...
                                        <div data-v-5437da4f="" class="flex gap-x-5 whitespace-nowrap"><?= $flight['date_depart'] ?> <span data-v-5437da4f="" class="text-gray-light">|</span><?= $nbrpass ?> Passager</div>
                        ...
                        <div class="container" style=" max-width: 400px;margin: 0 auto;padding: 20px;text-align: center;margin-left: 200px;">
                            <h4 style="font-weight: bold;">Décalage horaire</h4>
                            <div id="franceTime" style=" margin-bottom: 10px;"></div>
                            <div id="algeriaTime" style=" margin-bottom: 10px;"></div>
                        </div>
                        <div data-v-5437da4f="" class="flex items-center"><button data-v-5437da4f="" class="flex items-center justify-center px-15 rounded font-bold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-opacity-50 bg-primary text-white hover:bg-primary-dark active:bg-primary-dark focus:ring-primary h-[49px] text-base change-button w-[161px] flex flex-col bg-opacity-20 z-10 !text-primary hover:!text-white h-[47px]" data-testid="flightResult-changeFlight-button" type="button"><!---->
                        ...
                        <div data-v-0c95760a="" class="sticky-sidebar" style="transform: unset; top: unset;">
                            <div data-v-7a895338="" class="overflow-x-hidden px-5 duration-200">
                                <div data-v-7a895338="" class="flex items-center bottom-0 w-full relative">
                                    ...
                                    <a data-v-7a895338="" variant="clear" data-testid="filter-reset-textButton" href="index.php" class="text-primary absolute right-0 cursor-pointer" id="resetbtn">Réinitialiser
                                        tout</a>
                                </div>
                                ...
                                <input type="submit" value="Rechercher" name="chercher" class="book">
                            </div>
                        </div>
                    </div>
                </aside>
                        ...
                        <div data-v-b5a9bae3="" class="w-full flex flex-col gap-y-10">
                            <div data-v-b5a9bae3="" class="flex flex-row justify-between items-center">
                                ...
                                <div data-v-b5a9bae3="" class="flex gap-x-5">
                                    <p data-v-b5a9bae3=""><?= $flight['date_depart'] ?></p> | <p data-v-b5a9bae3=""><?= $nbrpass ?> Passager</p>
                                </div>
                            </div>
...
<script src="js/pageVol.js"></script>
<script>
    function afficherHeure() {
        // Récupération de l'heure actuelle en France
        var AlgeriaTime = new Date();
        var algeriaHours = AlgeriaTime.getHours();
        var algeriaMinutes = AlgeriaTime.getMinutes();
        var algeriaSeconds = AlgeriaTime.getSeconds();

        // Calcul du décalage horaire entre la France et l'Algérie (1 heure)
        var franceHours = algeriaHours + 1;
        if (franceHours >= 24) {
            franceHours -= 24;
        }

        // Affichage de l'heure en Algerie
        document.getElementById("algeriaTime").innerHTML = "Heure en Algérie : " + algeriaHours + ":" + pad(algeriaMinutes) + ":" + pad(algeriaSeconds);

        // Affichage de l'heure en France
        document.getElementById("franceTime").innerHTML = "Heure en  France : " + franceHours + ":" + pad(algeriaMinutes) + ":" + pad(algeriaSeconds);
    }

    // Ajout d'un zéro devant les nombres < 10
    function pad(number) {
